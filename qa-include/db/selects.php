...
function qa_db_tag_recent_qs_selectspec($voteuserid, $tag, $start, $full = false, $count = null)
{
	...
	//STW_EDIT
	$selectspec['source'] .= " JOIN (SELECT postid FROM ^posttags WHERE wordid=(SELECT wordid FROM ^words WHERE translit=$ AND translit=$ COLLATE utf8_bin LIMIT 1) ORDER BY postcreated DESC LIMIT #,#) y ON ^posts.postid=y.postid";
...
function qa_db_tag_word_selectspec($tag)
{
	return array(
		...
		//STW_EDIT
		'source' => '^words WHERE translit=$ AND translit=$ COLLATE utf8_bin',
